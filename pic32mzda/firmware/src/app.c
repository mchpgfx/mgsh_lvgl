...
/* Next unused section in DDR (after GLCD frame buffer) is used for LVGL buffers. */
#define SCRATCH_BUFFER_DRAM 0xA8465000
...
static void lv_disp_drv_flush_cb(lv_disp_drv_t * disp_drv, const lv_area_t * area, lv_color_t * color_p) 
{
    ...
    DRV_GLCD_BlitBuffer(area->x1, area->y1, &dstBuf);
...
void APP_Tasks ( void )
{
    ...
    switch ( appData.state )
    {
        ...
        case APP_STATE_INIT:
        {
            ...
            DRV_GLCD_IOCTL(GFX_IOCTL_SET_LAYER_COLOR_MODE, &ioctlArg);
            
            /* Register Display in LVGL */
            lv_disp_draw_buf_init(&lv_disp_draw_buf,
                                  (void*) SCRATCH_BUFFER_DRAM,
                                  NULL,
                                  DISP_HRES * DISP_VRES);
