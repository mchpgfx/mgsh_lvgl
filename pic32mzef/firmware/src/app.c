...
#define DISP_BUFF_HRES 100
#define DISP_BUFF_VRES 100
...
/* Scratch Buffer */
__attribute__ ((section(".region_nocache"), aligned (32))) uint16_t scratch_buff[DISP_BUFF_HRES * DISP_BUFF_VRES];
...
static void lv_disp_drv_flush_cb(lv_disp_drv_t * disp_drv, const lv_area_t * area, lv_color_t * color_p) 
{
    ...
    DRV_LCC_BlitBuffer(area->x1, area->y1, &dstBuf);
...
void APP_Tasks ( void )
{
    ...
    switch ( appData.state )
    {
        ...
        case APP_STATE_INIT:
        {
            ...
            DRV_LCC_IOCTL(GFX_IOCTL_SET_ACTIVE_LAYER, &ioctlArg);  
            ioctlArg.value.v_colormode = GFX_COLOR_MODE_RGB_565;
            DRV_LCC_IOCTL(GFX_IOCTL_SET_LAYER_COLOR_MODE, &ioctlArg);
            
            /* Register Display in LVGL */
            lv_disp_draw_buf_init(&lv_disp_draw_buf,
                                  scratch_buff,
                                  NULL,
                                  DISP_BUFF_HRES * DISP_BUFF_VRES);
            
            lv_disp_drv_init(&lv_disp_drv);
            lv_disp_drv.hor_res = DISP_HRES;
            lv_disp_drv.ver_res = DISP_VRES;
